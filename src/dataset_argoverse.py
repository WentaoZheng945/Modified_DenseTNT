...
def get_single_agent_instance(track_id, scenario, argoverse2_map, to_predict_track, args:utils.Args, file_name):
    from av2.datasets.motion_forecasting import data_schema
    from av2.datasets.motion_forecasting.data_schema import ObjectType

    object_type_to_int = defaultdict(int)
    object_type_to_int[ObjectType.VEHICLE] = 1  # "vehicle"
    object_type_to_int[ObjectType.PEDESTRIAN] = 2  # "pedestrian"
    object_type_to_int[ObjectType.MOTORCYCLIST] = 3  # "motorcyclist"
    object_type_to_int[ObjectType.CYCLIST] = 4  # "cyclist"
    object_type_to_int[ObjectType.BUS] = 5  # "bus"
    ...
    if True:
        ...
        tracks = [focal_track] + tracks  # focal_track放在最前面
    ...
    if True:
        ...
        # print(len(focal_track.object_states))
        ...
        for timestep, state in enumerate(focal_track.object_states):
            ...
            # current timestep
            if state.timestep == 50 - 1:  # 前50帧作为输入
                cent_x = state.position[0]
                cent_y = state.position[1]
                angle = -state.heading + math.radians(90)  # TODO
                normalizer = utils.Normalizer(cent_x, cent_y, angle)  # 老坐标系原点在新坐标系下的坐标
            elif state.timestep >= 50:
                labels.append(normalizer((state.position[0], state.position[1])))  # 计算其在新坐标系下的坐标
                gt_trajectory_global_coordinates.append((state.position[0], state.position[1]))  # 计算其在老坐标系下的坐标
    ...
    # key为track_id, value为每个车的轨迹，也是一个dict，dict有两个key，一个是'past', 一个是future，value每个也是一个dict，key为时间，value为[x, y], 且为局部坐标下的
    ...
    for track in tracks:
        ...
        agent = []
        past_dict = {}  # 记录过去轨迹，key为时间，value为[x, y]
        future_dict = {}  # 同上
        timestep_to_state = {}  # 每条track的时间戳和state字典
        for state in track.object_states:
            if state.observed:
                assert state.timestep < 50
                timestep_to_state[state.timestep] = state
                agent.append(normalizer([state.position[0], state.position[1]]))  # 每辆车在新坐标系下的坐标
                past_dict[state.timestep] = normalizer([state.position[0], state.position[1]])
            else:
                assert state.timestep >= 50
                future_dict[state.timestep] = normalizer([state.position[0], state.position[1]])
        ...
        while i < 50:
            if i in timestep_to_state:
                ...
                vector = np.zeros(args.hidden_size)  # 128
        ...
        for lane_segment in argoverse2_map.vector_lane_segments.values():
            ...
            for waypoints in [lane_segment.left_lane_boundary.waypoints, lane_segment.right_lane_boundary.waypoints]:
                ...
                for point in waypoints:
                    polyline.append(normalizer([point.x, point.y]))  # 左、右边界
                polyline = np.array(polyline)
                polygons.append(polyline)  # 每个元素是一个lane_segment的左边界或者右边界
        ...
        if 'goals_2D' in args.other_params:
            ...
            for index_polygon, polygon in enumerate(polygons):
                for i, point in enumerate(polygon):
                    ...
                    if hash not in visit:
                        visit[hash] = True
                        points.append(point)  # 把地图的点收集起来

                # Subdivide lanes to get more fine-grained 2D goals.
                if 'subdivide' in args.other_params:
                    subdivide_points = get_subdivide_points(polygon)  # 拆成更细的颗粒度
                    points.extend(subdivide_points)
    ...
    if 'goals_2D' in args.other_params:
        point_label = np.array(labels[-1])  # 终点坐标
        mapping['goals_2D_labels'] = np.argmin(get_dis(mapping['goals_2D'], point_label))  # 采样的goals_2D里面里终点最近的点的索引
        ...
        if 'lane_scoring' in args.other_params:
            ...
            mapping['stage_one_label'] = stage_one_label  # 终点坐标所在的车道的索引
    ...
    to_predict_track[track_id] = mapping


def argoverse2_get_instance_eval(args: utils.Args, instance_dir):
    from av2.datasets.motion_forecasting import data_schema
    from av2.datasets.motion_forecasting.data_schema import ObjectType
    ...
    def __init__(self, args, batch_size, to_screen=True):
        ...
        if args.reuse_temp_file:
            ...
            if args.core_num >= 1:
                ...
                # pickle_file.close()
                # files = [file for file in files if file.split('/')[-1] in useful_name]

                pbar = tqdm(total=len(files))
                print('Number of training dataset:', len(files))
                ...
                print("\033[31m" + 'Put each scenario into the task queue' + "\033[0m")
                for file in files:
                    assert file is not None
                    queue.put(file)
                    pbar.update(1)
                print("\033[31m" + 'Finish Putting each scenario into the task queue' + "\033[0m")
                ...
                self.ex_list = []
                print("\033[31m" + 'Record the results of preprocessing' + "\033[0m")
                pbar = tqdm(total=len(files))
                for i in range(len(files)):
                    ...
                    pbar.update(1)
                pbar.close()
                print("\033[31m" + 'Finish recording the results of preprocessing' + "\033[0m")
...
class DatasetTrain(torch.utils.data.Dataset):
    def __init__(self, args, batch_size, to_screen=True):
        ...
        if args.reuse_temp_file:  # 是否用已经处理的数据
            # pickle_file = open(os.path.join(args.temp_file_dir, get_name('ex_list')), 'rb')
            pickle_file = open(os.path.join('/root/autodl-tmp/data/preprocess_data', get_name('ex_list')), 'rb')
            self.ex_list = pickle.load(pickle_file)
            ...
            if args.core_num >= 1:
                ...
                queue = multiprocessing.Queue(args.core_num)  # 未处理的任务 消息队列，core_num是同时处理的任务数量
                queue_res = multiprocessing.Queue()  # 处理完返回的结果

                print("\033[31m" + 'Put each scenario into the task queue and preprocess each scenario' + "\033[0m")
                    ...
                    while True:
                        ...
                        def put_instance_in_queue(instance):
                            if instance is not None:
                                data_compress = zlib.compress(pickle.dumps(instance))
                                res.append(data_compress)
                                queue_res.put(data_compress)  # 54931
                            else:
                                queue_res.put(None)

                        if args.argoverse2:
                            instance = argoverse2_get_instance_eval(args, file)
                            if isinstance(instance, list):
                                for each_instance in instance:
                                    counter.value += 1
                                    put_instance_in_queue(each_instance)
                            else:
                                counter.value += 1
                                put_instance_in_queue(instance)
                        else:
                ...
                condition_1 = 0
                condition_2 = 0
                while condition_1 < counter.value or  condition_2 < args.core_num:
                    t = queue_res.get()
                    if t is not None:
                        self.ex_list.append(t)
                        condition_1 += 1
                ...
                # 起double_check的作用，确保queue_res空了
                ...
                print("\033[m" + 'Checking Number of samples' + "\033[0m")
                pbar = tqdm(total=len(self.ex_list))
                for i in range(len(self.ex_list)):
                    pbar.update(1)
                pbar.close()
                print("\033[m" + 'Finish Checking Number of samples' + "\033[0m")
            else:
                assert False

            # pickle_file = open(os.path.join(args.temp_file_dir, get_name('ex_list')), 'wb')
            pickle_file = open(os.path.join('/root/autodl-tmp/data/preprocess_data', get_name('ex_list')), 'wb')
            pickle.dump(self.ex_list, pickle_file)
            pickle_file.close()
        assert len(self.ex_list) > 0
        if to_screen:
                    ...
                    while True:
                        ...
                        if args.argoverse2:
                            if args.do_train and not args.do_eval:
                                ...
                                raise ValueError("Training Mode Configuration Error")
                        else:
                            if file.endswith("csv"):
                                with open(file, "r", encoding='utf-8') as fin:
                                    lines = fin.readlines()[1:]
