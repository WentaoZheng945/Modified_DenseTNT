...
class GlobalGraph(nn.Module):
    ...
    def __init__(self, hidden_size, attention_head_size=None, num_attention_heads=1):
        ...
        self.num_attention_heads = num_attention_heads  # 头的数量
        self.attention_head_size = hidden_size // num_attention_heads if attention_head_size is None else attention_head_size  # 未指定平均分配
        self.all_head_size = self.num_attention_heads * self.attention_head_size  # 计算头的总大小
    ...
    def get_extended_attention_mask(self, attention_mask):
        ...
        即1是不需要关注的部分，0是需要关注的部分，即填充1的部分不做任何处理，对0的位置减一个很大的数
        """
        extended_attention_mask = attention_mask.unsqueeze(1)  # 对应位置升一维
    ...
    def forward(self, hidden_states, attention_mask=None, mapping=None, return_scores=False):
        ...
        mixed_key_layer = nn.functional.linear(hidden_states, self.key.weight)  # 一个特殊处理，相比transformer不使用key的偏执项
        mixed_value_layer = self.value(hidden_states)

        query_layer = self.transpose_for_scores(mixed_query_layer)  # Q
        key_layer = self.transpose_for_scores(mixed_key_layer)  # K
        value_layer = self.transpose_for_scores(mixed_value_layer)  # V

        attention_scores = torch.matmul(
            query_layer / math.sqrt(self.attention_head_size), key_layer.transpose(-1, -2))  # (batch, head, max_vector_num, max_vector_num)
        ...
        attention_probs = nn.Softmax(dim=-1)(attention_scores)  # (batch, head, max_vector_num, max_vector_num)
        ...
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()  # (batch, max_vector_num, head, head_size)
        new_context_layer_shape = context_layer.size()[
                                  :-2] + (self.all_head_size,)  # (batch, max_vector_num, head*head_size)
...
class CrossAttention(GlobalGraph):
    ...
    def forward(self, hidden_states_query, hidden_states_key=None, attention_mask=None, mapping=None,
        ...
        return context_layer  # (batch, max_vector_num, head*head_size)
...
class GlobalGraphRes(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, mapping=None):
        ...
        hidden_states = torch.cat([self.global_graph(hidden_states, attention_mask, mapping),
                                   self.global_graph2(hidden_states, attention_mask, mapping)], dim=-1)  # # (batch, max_vector_num, 64)
        return hidden_states  # # (batch, max_vector_num, 128)
...
class PointSubGraph(nn.Module):
    ...
    def forward(self, hidden_states: Tensor, agent: Tensor):
        ...
        Args:
            hidden_states: 每个预测目标的终点坐标候选 shape (1, goal_num, 2)
            agent: shape (1, 128)
        ...
        predict_agent_num, point_num = hidden_states.shape[0], hidden_states.shape[1]  # 预测的agent的数量，点的数量 (1 goal_num)
        hidden_size = self.hidden_size  # 128
        assert (agent.shape[0], agent.shape[1]) == (predict_agent_num, hidden_size)
        agent = agent[:, :hidden_size // 2].unsqueeze(1).expand([predict_agent_num, point_num, hidden_size // 2])  # agent的shape为(1, goal_num, 64)
        for layer_index, layer in enumerate(self.layers):
            if layer_index == 0:
                hidden_states = layer(hidden_states)  # 输入为(1, goal_num, 2) 输出为(1, goal_num, 64)
            else:
                hidden_states = layer(torch.cat([hidden_states, agent], dim=-1))  # 输入为(1, goal_num, 128) 输出为(1, goal_num, 128)

        return hidden_states  # (1, goal_num, 128)
