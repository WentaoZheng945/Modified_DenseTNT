...
def add_argument(parser):
    ...
    parser.add_argument("--do_train",
                        action='store_true',
                        help="Whether to run training.")  # 是否训练 default: True
    ...
    parser.add_argument("--data_dir",
                        default='/root/autodl-tmp/data/train',
                        type=str)  # 训练数据的目录 default: data/train
    parser.add_argument("--data_dir_for_val",
                        default='/root/autodl-tmp/data/val',
                        type=str)  # 验证数据的目录 default: data/val
    parser.add_argument("--output_dir", default="tmp/", type=str)  # 输出目录 argoverse2.densetnt.1
    ...
    parser.add_argument("--train_batch_size",
                        default=64,
                        type=int,
                        help="Total batch size for training.")  # 训练批次的大小 default: 64

    parser.add_argument("--eval_batch_size",
                        default=64,
                        type=int,
                        help="Total batch size for eval.")  # 评估批次的大小 default: 64
    parser.add_argument("--model_recover_path",
                        default=None,
                        type=str)  # 模型恢复的路径 None
    parser.add_argument("--learning_rate", default=0.001, type=float,
                        help="The initial learning rate for Adam.")  # 学习率 0.001
    parser.add_argument("--weight_decay",
                        default=0.01,
                        type=float,
                        help="The weight decay rate for Adam.")  # adam优化器的权重衰减率 0.01
    parser.add_argument("--num_train_epochs",
                        default=16.0,
                        type=float,
                        help="Total number of training epochs to perform.")  # 训练的总轮数 16
    ...
    parser.add_argument("--no_cuda",
                        action='store_true',
                        help="Whether not to use CUDA when available")  # 指使是否不使用cuda False
    parser.add_argument("--hidden_size",
                        default=64,
                        type=int)  # 隐藏层大小 128
    parser.add_argument("--hidden_dropout_prob",
                        default=0.1,
                        type=float)  # dropout的概率 0.1
    ...
    parser.add_argument("--debug",
                        action='store_true')  # 是否开启调试模式 False
    ...
    parser.add_argument("--not_use_api",
                        action='store_true')  # 是否不使用API False
    ...
    parser.add_argument("--train_extra",
                        action='store_true')  # 是否训练额外的模型 False
    parser.add_argument("--use_centerline",
                        action='store_true')  # 是否使用中心线 True
    ...
    parser.add_argument("--lstm",
                        action='store_true')  # 是否使用lstm False
    parser.add_argument("--add_prefix",
                        default=None)  # 是否添加前缀 None
    parser.add_argument("--attention_decay",
                        action='store_true')  # 是否使用注意力衰减 False
    ...
    parser.add_argument("--stage_one_K", type=int)  # 第一阶段的k值 None
...
def init(args_: Args, logger_):
    ...
    # 如果不是做eval的话，这里会提示一下，已经存在对应的文件夹
    ...
    if args.log_dir is None:
        args.log_dir = args.output_dir  # log_dir就为输出目录
    if args.temp_file_dir is None:
        args.temp_file_dir = os.path.join(args.output_dir, 'temp_file')  # 临时目录为 argoverse2.densetnt.1/temp_file
    else:
    ...
    os.makedirs(args.output_dir, exist_ok=True)  # tmp/
    os.makedirs(args.log_dir, exist_ok=True)  # tmp/
    os.makedirs(args.temp_file_dir, exist_ok=True)  # tmp/temp_file
    ...
    def init_args_do_eval():
        """
        若评估需要确定下是用哪个数据集的位置，如使用val数据，则把do_eval设为True，do_test为False
                                       如使用test数据，则把do_eval设为True，do_test也设为True
        Returns:

        """
        if args.argoverse:
            args.data_dir = args.data_dir_for_val if not args.do_test else '/root/autodl-tmp/data/test/'
    ...
    print("\033[31m" + 'Configuration Parameters for this training' + "\033[0m")
...
def logging(*inputs, prob=1.0, type='1', is_json=False, affi=True, sep=' ', to_screen=False, append_time=False, as_pickle=False):
    ...
    Args:
        *inputs: 需要记录的内容
        ...
        affi:
        sep: 打印输出的分隔符
        to_screen: 是否同时输出到屏幕上
        append_time: 日志文件名是否追加时间
        as_pickle: 是否讲输入序列序列化为pickle
    ...
    if as_pickle:
        ...
        return
    if file not in files_written:  # 如果这个文件还没被创建，则创建，改mode为写入
    ...
    for i, each in enumerate(inputs):
        if isinstance(each, torch.Tensor):
            ...
            the_tensor = inputs[i]
    np.set_printoptions(threshold=np.inf)  # 设置可以打印numpy的大数组
...
mpl.use('Agg')  # 设置matplotlib的后端为agg
...
def visualize_goals_2D(mapping, goals_2D, scores: np.ndarray, future_frame_num, loss=None, labels: np.ndarray = None,
                       labels_is_valid=None, predict: np.ndarray = None, extra = False):
    """

    Args:
        mapping:
        goals_2D: ndarray shape (dense_goal_nums, 2)
        scores: 每个终点的得分
        future_frame_num: 60
        loss: 损失
        labels: shape(60, 2)
        labels_is_valid: groud truth是否可用，shape(60, )
        predict: 每个场景的预测结果 ndarray shape为(6, 60, 2)
    ...
    visualize_num += 1
    if visualize_num > args.visualize_num and 'vis_video' not in args.other_params and 'vis_all' not in args.other_params:
        return
        # print('press any key to continue')
        # input()
    print('in visualize_goals_2D', mapping['file_name'])  # 场景名称
    print('speed', mapping.get('seep', None))  # 速度估计没有

    assert predict is not None
    predict = predict.reshape([6, future_frame_num, 2])
    assert labels.shape == (future_frame_num, 2)

    if 'eval_time' in mapping:  # 60
        assert labels.shape[0] == labels_is_valid.shape[0] == future_frame_num
        eval_time = mapping['eval_time']  # 60
    ...
    target_agent_color, target_agent_edge_color = '#4bad34', '#c5dfb3'  # 目标车和目标车边缘的颜色（绿色）
    ...
    if True:
        plt.xlim(-100, 100)  # 显示范围
        plt.ylim(-30, 100)

    # plt.figure(0, dpi=300)
    cmap = plt.cm.get_cmap('Reds')  # 一个名为Reds的颜色映射
    vmin = np.log(0.0001)
    vmin = np.log(0.00001)  # 将分数的最小值设置为0.00001
    scores = np.clip(scores.copy(), a_min=vmin, a_max=np.inf)
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=plt.Normalize(vmin=vmin, vmax=np.max(scores)))  # 设定颜色对应的数值范围
    plt.colorbar(sm)  # 画出bar

    trajs = mapping['trajs']  # 所有车前50帧的信息
    if extra:
        full_trajectory = mapping['full_trajectory']

    if args.argoverse:
        name = os.path.split(mapping['file_name'])[1].split('.')[0]
    # name = name + '.FDE={}'.format(loss)
    if extra:
        name = name + '.track id: {0}'.format(mapping['track_id'])

    add_end = True  # 是否绘制轨迹终点
    ...
    for lane in mapping['vis_lanes']:
        ...
        # 黑色实线，不标记，0.5的透明度，线宽为2，zorder设置为0（即在背景层也就是最底层绘制）
    ...
    def draw_trajectory(full_trajectory):
        for id, traj in full_trajectory.items():
            if traj['category'].name not in ['VEHICLE', 'PEDESTRIAN', 'MOTORCYCLE', 'CYCLIST', 'BUS']:
                continue
            if id == mapping['track_id']:
                past_trajectory = list(traj['past'].values())
                past_trajectory = np.array(past_trajectory).reshape([-1])
                t = np.zeros(len(past_trajectory)+2)
                t[:len(past_trajectory)] = past_trajectory
                t[-2] = labels[0]  # 未来真实第一帧坐标
                t[-1] = labels[1]
                # 绘制目标车历史轨迹，实线，不透明，最底层
                plt.plot(t[0::2], t[1::2], linestyle="-", color=target_agent_color, marker='o',
                         alpha=1,
                         linewidth=linewidth,
                         markersize=3,
                         zorder=2)  # 预测车过去轨迹
                # plt.scatter(t[0::2], t[1::2], color=target_agent_color, s=marker_size_scale, alpha=1, zorder=0)
                heading1 = np.arctan2(past_trajectory[3] - past_trajectory[1],
                                     past_trajectory[2] - past_trajectory[0])
                heading2 = np.arctan2(past_trajectory[5] - past_trajectory[3],
                                      past_trajectory[4] - past_trajectory[2])
                heading = (heading1 + heading2) / 2
                past_trajectory = past_trajectory.reshape([-1, 2])
                future_trajectory = np.array(list(traj['future'].values())).reshape([-1, 2])
                full_trajectory = np.concatenate((past_trajectory, future_trajectory), axis=0)
            else:  # 其他车
                past_trajectory = np.array(list(traj['past'].values())).reshape([-1, 2])
                future_trajectory = np.array(list(traj['future'].values())).reshape([-1, 2])
                if len(past_trajectory) == 0:
                    full_trajectory = future_trajectory
                elif len(future_trajectory) == 0:
                    full_trajectory = past_trajectory
                else:
                    full_trajectory = np.concatenate((past_trajectory, future_trajectory), axis=0)
                if len(full_trajectory) >= 40:
                    if get_L2_dis(full_trajectory):
                        color = "darkblue"
                        plt.plot(full_trajectory[:, 0], full_trajectory[:, 1], linestyle="-", color=color, marker='o',
                                 alpha=1,
                                 linewidth=linewidth,
                                 markersize=3,
                                 zorder=2)
                        # plt.scatter(full_trajectory[:, 0], full_trajectory[:, 1], color=color, s=marker_size_scale, alpha=1, zorder=0)
                heading1 = np.arctan2(full_trajectory[1][1] - full_trajectory[0][1],
                                      full_trajectory[1][0] - full_trajectory[0][0])
                heading2 = np.arctan2(full_trajectory[2][1] - full_trajectory[1][1],
                                      full_trajectory[2][0] - full_trajectory[1][0])
                heading = (heading1 + heading2) / 2
                # heading = -(traj['heading'] - math.radians(90))
                # heading = traj['heading']

            # 画出vehicle的形状
            # TODO （1）车边框的zorder调整到地图上面 （2）再研究下heading
            if len(full_trajectory) <= 40:
                continue
            if not get_L2_dis(full_trajectory):
                continue
            if traj['category'].name == 'VEHICLE':
                ...
                continue
                # print(traj['category'].name)
            if traj['category'].name == 'VEHICLE':
                rectangle = patches.Polygon(
                    vehicle_box,
                    closed=True,  # 是否封闭
                    facecolor='blue',  # 内部填充为蓝色
                    edgecolor='blue',  # 边缘颜色为绿色
                    linewidth=2,  # 边缘宽度
                    alpha=1.0,  # 不透明度
                    zorder=2
                )
                ...
                rectangle = patches.Polygon(
                    vehicle_box,
                    closed=True,  # 是否封闭
                    facecolor='yellow',  # 内部填充为蓝色
                    edgecolor='yellow',  # 边缘颜色为绿色
                    linewidth=2,  # 边缘宽度
                    alpha=1.0,  # 不透明度
                    zorder=2
                )
            ...
            if id == mapping['track_id']:
                ax.text(full_trajectory[0][0], full_trajectory[0][1], f"{traj['category'].name}", color='black', fontsize=10, ha='center', va='center')
            # ax.text(full_trajectory[0][0], full_trajectory[0][1], f"{heading}", color='black',
    ...
    def draw_his_trajs():
        for i, traj in enumerate(trajs):
            ...
            if i == 0:
                traj = np.array(traj).reshape([-1])  # 变成一维 100
                t = np.zeros(len(traj) + 2)  # 102
                t[:len((traj))] = traj
                t[-2] = labels[0]  # 第一帧的x坐标
                t[-1] = labels[1]  # 第一帧的y坐标
                # 绘制目标车历史轨迹，实线，不透明，最底层
    ...
    if extra:
        draw_trajectory(full_trajectory)
    else:
        draw_his_trajs()
    ...
    if True:
        if goals_2D is not None:
            ...
            marker_size = 70
            plt.scatter(goals_2D[:, 0], goals_2D[:, 1], c=scores, cmap=cmap, norm=sm.norm, s=marker_size, alpha=0.3, marker=',', zorder=1)
        # s is size, default 20

        # if False:
        for each in predict:
            # 预测轨迹，黑橙色
            function2 = plt.plot(each[:, 0], each[:, 1], linestyle="-", color="darkorange", marker='o',
                                 linewidth=linewidth,
                                 zorder=2, label='Predicted trajectory')
            # plt.scatter(each[:, 0], each[:, 1], color="darkorange", s=marker_size_scale, alpha=1, zorder=0)
            # 添加预测的终点
            if add_end:
        ...
        # 真实轨迹
        function1 = plt.plot(labels[0::2], labels[1::2], linestyle="-", color=target_agent_color, marker='o', linewidth=linewidth,
                             zorder=2, label='Ground truth trajectory')

    functions = function1 + function2
    fun_labels = [f.get_label() for f in functions]  # 从每个函数的句柄中获得标签
    plt.legend(functions, fun_labels, loc=0)  # 0表示显示在最佳位置
...
def run_process(queue, queue_res, args):
    ...
    if 'MRminFDE' in args.other_params:
        objective = 'MRminFDE'  # 执行
    opti_time = float(args.other_params.get('opti_time', 10000.0))  # 0.1
    ...
    while True:
        ...
        if 'MRminFDE' in args.other_params:
            ...
            MRratio = float(args.other_params['MRminFDE']) if args.other_params['MRminFDE'] is not True else 1.0  # 0.0
        ...
        if 'cnt_sample' in args.other_params:
            num_step = 1000
            kwargs.update(dict(
                num_step=num_step,  # 1000
                cnt_sample=args.other_params['cnt_sample'],  # 9
                MRratio=MRratio,  # 0.0
            ))
...
class PredictionMetrics:
    def __init__(self):
        self.minADE = Metric()  # average
        self.minFDE = Metric()  # final
        self.MR = Metric()  # missing rate
        self.brier_minFDE = Metric()
        self.minFDE_1 = Metric()
        self.minFDE_3 = Metric()

    def serialize(self) -> Dict[str, Any]:
        return dict(
            minADE=self.minADE.get_mean(),
            minFDE_1=self.minFDE_1.get_mean(),
            minFDE_3=self.minFDE_3.get_mean(),
